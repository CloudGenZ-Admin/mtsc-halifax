...
export default function Publication() {
  ...
  const getPdfUrl = (field, fallback = null) => {
    ...
    if (typeof field === 'object' && field !== null) {
      return getMediaUrl(field, fallback);
    }
  ...
  // Resolved Media URLs (Zero hardcoded image fallbacks)
  const heroBgImageUrl = getMediaUrl(data?.hero_bg_image, null);
  const card1ImageUrl = getMediaUrl(data?.card_1_image, null);
  const card2ImageUrl = getMediaUrl(data?.card_2_image, null);

  // 1. Halifax Newsletters Data (CMS only)
  const halifaxNewslettersData = (Array.isArray(data?.halifax_newsletters) && data.halifax_newsletters.length > 0)
    ? data.halifax_newsletters.map(item => ({
        title: item.title || "",
        href: getPdfUrl(item.pdf_file || item.pdf_url, null)
      }))
    : [];
  ...
  const activeHalifaxDisplay = currentHalifax.length > 0 ? currentHalifax : halifaxNewslettersData;

  // 2. Happiness Reports Data (CMS only)
  const happinessReportsData = (Array.isArray(data?.happiness_reports) && data.happiness_reports.length > 0)
  ...
  const activeHappinessDisplay = currentHappiness.length > 0 ? currentHappiness : happinessReportsData;

  // 3. Foghorn Reports Data (CMS only)
  const foghornReportsData = (Array.isArray(data?.foghorn_reports) && data.foghorn_reports.length > 0)
  ...
  const activeFoghornDisplay = currentFoghorn.length > 0 ? currentFoghorn : foghornReportsData;

  // 4. ESG Strategy Data (CMS only)
  const esgStrategyData = (Array.isArray(data?.esg_reports) && data.esg_reports.length > 0)
  ...
  const activeEsgDisplay = currentEsg.length > 0 ? currentEsg : esgStrategyData;

  // 5. Marine Safety Data (CMS only)
  const marineSafetyData = (Array.isArray(data?.safety_handbooks) && data.safety_handbooks.length > 0)
  ...
  const activeSafetyDisplay = currentSafety.length > 0 ? currentSafety : marineSafetyData;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 overflow-x-hidden w-full">
      ...
      <main className="flex-grow">
        ...
        <section className="relative pt-24 pb-32 overflow-hidden bg-[#112A46]">
          {heroBgImageUrl && (
            <div 
              ...
              style={{ backgroundImage: `url(${heroBgImageUrl})`, animation: 'subtle-zoom 20s ease-in-out infinite alternate' }}
            ></div>
          )}
          ...
          <div className="max-w-[1200px] mx-auto px-7 relative z-10 text-center flex flex-col items-center mt-8">
            {data?.hero_badge && (
              ...
              >
                <FaBookOpen /> {data.hero_badge}
              </span>
            )}
            {(data?.hero_title || data?.hero_title_coral) && (
              <h1 className="text-[clamp(36px,5vw,56px)] font-black text-white leading-[1.1] mb-6 max-w-4xl mx-auto drop-shadow-2xl">
                {data?.hero_title}{' '}
                {data?.hero_title_coral && <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#E05A2B] to-[#ff9770]">{data.hero_title_coral}</span>}{' '}
                {data?.hero_title_suffix}
              </h1>
            )}
            {data?.hero_subtitle && (
        ...
        {(data?.halifax_title || halifaxNewslettersData.length > 0) && (
            ...
            <div className="max-w-[1200px] mx-auto px-7 relative z-10">
              <Reveal>
                <div className="border-l-4 border-[#E05A2B] pl-6 mb-12">
                  ...
                  {data?.halifax_subtitle && <p className="text-gray-500 font-medium">{data.halifax_subtitle}</p>}
                </div>
                
                {activeHalifaxDisplay.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
                    {activeHalifaxDisplay.map((newsletter, index) => (
                      <PdfCard key={index} title={newsletter.title} href={newsletter.href} />
                    ))}
                  </div>
                )}

                {archiveHalifax.length > 0 && (
                ...
                {showArchivesHalifax && archiveHalifax.length > 0 && (
        ...
        {(data?.sea_title || data?.sea_description) && (
            ...
            <div className="max-w-[1200px] mx-auto px-7 relative z-10">
              <Reveal className="bg-white/60 backdrop-blur-2xl border border-white/50 rounded-[2rem] p-8 md:p-14 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-10 shadow-[0_20px_50px_rgba(224,90,43,0.08)]">
                ...
                <div className="relative z-10 max-w-2xl">
                  {data?.sea_badge && <div className="inline-block bg-[#E05A2B]/10 text-[#E05A2B] px-4 py-1.5 rounded-full font-bold text-sm mb-4">{data.sea_badge}</div>}
                  {data?.sea_title && (
                    <h2 className="text-[36px] font-black text-[#112A46] mb-4 leading-tight whitespace-pre-line">
                      {data.sea_title}
                    </h2>
                  )}
                  {data?.sea_description && (
                ...
                <div className="relative z-10 flex flex-col w-full md:w-auto shrink-0 gap-4">
                  {data?.btn_sea_global_text && (
                    <a
                      href={data?.btn_sea_global_link || "#"}
                      target="_blank"
                      rel="noopener noreferrer"
                    ...
                    >
                      <FaEnvelopeOpenText className="group-hover:-translate-y-1 group-hover:scale-110 transition-all" /> {data.btn_sea_global_text}
                    </a>
                  )}
                  {data?.btn_sea_canada_text && (
                    <a
                      href={data?.btn_sea_canada_link || "#"}
                    ...
                    >
                      <FaEnvelopeOpenText className="group-hover:-translate-y-1 group-hover:scale-110 transition-all" /> {data.btn_sea_canada_text}
                    </a>
                  )}
                  {data?.btn_sea_archive_text && (
                    <a
                      href={data?.btn_sea_archive_link || "#"}
                      target="_blank"
                      rel="noopener noreferrer"
                    ...
                    >
                      <FaBookOpen /> {data.btn_sea_archive_text}
                    </a>
                  )}
                </div>
              </Reveal>
        ...
        {/* 3. FAN */}
        {(data?.fan_title_prefix || data?.fan_description) && (
            ...
            <div className="max-w-[1100px] mx-auto px-7 relative z-10">
              <Reveal className="relative bg-[#112A46] rounded-[2.5rem] p-10 md:p-16 shadow-[0_30px_60px_rgba(17,42,70,0.4)] border border-[#1a3a5f] flex flex-col md:flex-row items-center justify-between gap-12 overflow-hidden group">
                ...
                <div className="relative z-10 md:w-2/3 text-center md:text-left">
                  ...
                  {data?.fan_title_prefix && (
                    <h2 className="text-[20px] md:text-[46px] font-black text-white mb-6 leading-tight drop-shadow-lg tracking-tight">
                      {data.fan_title_prefix}{' '}
                      {data?.fan_title_coral && <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#E05A2B] to-[#ff9770]">{data.fan_title_coral}</span>}
                    </h2>
                  )}
                  
                  {data?.fan_description && (
                ...
                {data?.btn_fan_signup_text && (
                  <div className="relative z-10 md:w-1/3 flex justify-center md:justify-end w-full">
                    ...
                    >
                      {data.btn_fan_signup_text} <FaExternalLinkAlt className="group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1 transition-transform" />
                    </a>
                  </div>
                )}
              </Reveal>
        ...
        {/* 4. STATISTICS */}
        {(data?.stats_title || data?.stats_description) && (
            ...
            <div className="max-w-[1000px] mx-auto px-7 relative z-10">
              <Reveal className="flex flex-col md:flex-row gap-10 items-center bg-gradient-to-br from-white to-blue-50/50 p-10 rounded-[2rem] border border-blue-100 shadow-xl">
                <div className="flex-1">
                  ...
                  {data?.btn_stats_text && (
                    <a
                      href={getPdfUrl(data?.stats_pdf_file || data?.stats_pdf_url, "#")}
                      target="_blank"
                      rel="noopener noreferrer"
        ...
        {/* 5. HAPPINESS INDEX */}
        {(data?.happiness_title || happinessReportsData.length > 0) && (
              ...
              <Reveal>
                <div className="border-l-4 border-[#E05A2B] pl-6 mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div>
                    ...
                    {data?.happiness_subtitle && <p className="text-gray-500 font-medium">{data.happiness_subtitle}</p>}
                  </div>
                </div>

                {activeHappinessDisplay.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                    {activeHappinessDisplay.map((report, index) => (
                      <PdfCard key={index} title={report.title} href={report.href} />
                    ))}
                  </div>
                )}

                {archiveHappiness.length > 0 && (
                ...
                {showArchivesHappiness && archiveHappiness.length > 0 && (
                ...
                {data?.happiness_survey_text && (
                  <div className="bg-gradient-to-r from-[#112A46] via-[#1a3a5f] to-[#112A46] text-white p-8 rounded-2xl flex flex-col md:flex-row items-center gap-6 shadow-2xl hover:shadow-[0_20px_50px_rgba(17,42,70,0.3)] hover:-translate-y-1 transition-all duration-300 border border-white/10 group relative overflow-hidden">
                    ...
                    <div className="relative z-10 text-center md:text-left">
                      <a
                        href={data?.happiness_survey_link || "#"}
                        target="_blank"
                        rel="noopener noreferrer"
        ...
        {/* 6. ESG STRATEGY */}
        {(data?.esg_title || esgStrategyData.length > 0) && (
          <section id="esg-strategy" className="py-24 relative overflow-hidden bg-[conic-gradient(at_top_right,_var(--tw-gradient-stops))] from-green-50 via-white to-emerald-50 scroll-mt-[90px]">
            ...
            <div className="max-w-[800px] mx-auto px-7 text-center relative z-10">
              <Reveal className="bg-white/60 backdrop-blur-2xl border border-green-200 rounded-[2.5rem] p-10 md:p-14 shadow-[0_20px_50px_rgba(16,185,129,0.08)]">
                ...
                {data?.esg_title && <h2 className="text-[36px] font-black text-[#112A46] mb-4">{data.esg_title}</h2>}
                {data?.esg_subtitle && <p className="text-gray-600 mb-10 font-medium">{data.esg_subtitle}</p>}
                ...
                {showArchivesEsg && archiveEsg.length > 0 && (
        ...
        {/* 7. THE FOGHORN */}
        {(data?.foghorn_title || foghornReportsData.length > 0) && (
            ...
            <div className="max-w-[1000px] mx-auto px-7 relative z-10">
              <Reveal>
                <div className="border-l-4 border-[#E05A2B] pl-6 mb-12">
                  ...
                  {data?.foghorn_subtitle && <p className="text-gray-600 font-medium">{data.foghorn_subtitle}</p>}
                </div>


                {archiveFoghorn.length > 0 && (
                  <div className="mt-12 flex justify-center">
                    <button
                    ...
                    >
                      <FaBookOpen className="group-hover:scale-110 transition-transform" /> 
                      {showArchivesFoghorn ? "Hide Archives" : (data?.btn_foghorn_archive_text || "Foghorn Archives")}
                    </button>
                  </div>
                )}

                {showArchivesFoghorn && archiveFoghorn.length > 0 && (
                  <div className="mt-10 pt-10 border-t border-gray-200">
        ...
        {/* 8. MARINE SAFETY */}
        {(data?.safety_title || marineSafetyData.length > 0) && (
          <section id="marine-safety" className="py-28 relative overflow-hidden bg-gradient-to-br from-[#112A46] via-[#1a3a5f] to-[#0a192f] scroll-mt-[90px]">
            ...
            <div className="max-w-[1100px] mx-auto px-7 relative z-10">
                  ...
                  {archiveSafety.length > 0 && (
                    <div className="flex justify-center lg:justify-start">
                      ...
                      >
                        ...
                        {showArchivesSafety ? "Hide Archives" : (data?.btn_safety_archive_text || "Marine Safety Archives")}
                      </button>
                    </div>
                  )}
                </div>
              ...
              {showArchivesSafety && archiveSafety.length > 0 && (
        ...
        {(data?.industry_title || (Array.isArray(data?.industry_cards) && data.industry_cards.length > 0)) && (
            ...
            <div className="max-w-[1200px] mx-auto px-7 relative z-10">
              <Reveal>
                <div className="border-l-4 border-[#112A46] pl-6 mb-12">
                  ...
                  {data?.industry_subtitle && <p className="text-gray-500 font-medium">{data.industry_subtitle}</p>}
                </div>
                {Array.isArray(data?.industry_cards) && data.industry_cards.length > 0 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {data.industry_cards.map((card, idx) => (
                      <a
                        key={idx}
                        href={card.card_url || "#"}
                        target="_blank"
        ...
        {/* 10. MARITIME STORYTELLING SECTION */}
        {(data?.story_title || data?.card_1_title || data?.card_2_title || data?.cta_box_title) && (
          <section className="relative py-32 overflow-hidden bg-[#112A46]">
            ...
            <div className="max-w-[1200px] mx-auto px-7 relative z-10">
              ...
              {(data?.story_title || data?.story_subtitle) && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-16">
                ...
                {/* Story Card 1 */}
                {(data?.card_1_title || data?.card_1_desc || card1ImageUrl) && (
                  <Reveal>
                    <div className="bg-white/10 backdrop-blur-xl rounded-[2.5rem] overflow-hidden border border-white/20 shadow-2xl hover:shadow-[0_30px_60px_rgba(0,0,0,0.6)] transition-all duration-500 hover:-translate-y-3 group h-full flex flex-col">
                      {card1ImageUrl && (
                        <div className="aspect-[16/10] overflow-hidden relative">
                          ...
                          <img
                            src={card1ImageUrl}
                            alt={data?.card_1_title || "Story Card"}
                      ...
                      <div className="p-10 flex-grow">
                        <div className="flex items-center gap-4 mb-6">
                          ...
                          {data?.card_1_title && <h3 className="text-[26px] font-black text-white drop-shadow-md">{data.card_1_title}</h3>}
                        </div>
                        {data?.card_1_desc && (
                ...
                {/* Story Card 2 */}
                {(data?.card_2_title || data?.card_2_desc || card2ImageUrl) && (
                  <Reveal delay={100}>
                    <div className="bg-white/10 backdrop-blur-xl rounded-[2.5rem] overflow-hidden border border-white/20 shadow-2xl hover:shadow-[0_30px_60px_rgba(0,0,0,0.6)] transition-all duration-500 hover:-translate-y-3 group h-full flex flex-col">
                      {card2ImageUrl && (
                        <div className="aspect-[16/10] overflow-hidden relative">
                          ...
                          <img
                            src={card2ImageUrl}
                            alt={data?.card_2_title || "Research Card"}
                            className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"
                          />
                        </div>
                      )}
                      <div className="p-10 flex-grow">
                        <div className="flex items-center gap-4 mb-6">
                          ...
                          {data?.card_2_title && <h3 className="text-[26px] font-black text-white drop-shadow-md">{data.card_2_title}</h3>}
                        </div>
                        {data?.card_2_desc && (
                          <p className="text-blue-50/90 text-[16px] leading-relaxed">
                            {data.card_2_desc}
              ...
              {(data?.cta_box_title || data?.cta_box_desc) && (
                  ...
                  <div className="bg-white/10 backdrop-blur-2xl rounded-[3rem] p-10 md:p-16 border border-white/30 max-w-4xl mx-auto shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden group hover:border-white/50 transition-colors duration-500">
                    ...
                    <div className="flex flex-wrap justify-center gap-5 relative z-10">
                      {data?.btn_cta_1_text && (
                        ...
                        >
                          <FaBookOpen /> {data.btn_cta_1_text}
                        </a>
                      )}
                      {data?.btn_cta_2_text && (
                        <a
                          href={data?.btn_cta_2_link || "#"}
                          target="_blank"
                          rel="noopener noreferrer"
                        ...
                        >
                          <FaExternalLinkAlt /> {data.btn_cta_2_text}
                        </a>
                      )}
                    </div>
                  </div>
